...
$total_pages_ChoXacNhan = VanDon::countAllBuuCuc($pdo,$NhanVienDangNhap['idBC'], $limit, 'Chờ xác nhận');
...
if (isset($_GET['action']) && isset($_GET['idVD'])) {
    ...
    if (isset($Nhanvien)) {
        if ($action == 'xacnhan') {
            ...
            VanDon::XacNhanVD_BuuCuc($pdo, $idVD, 'Đang lấy', $Nhanvien['idNV'], $Nhanvien['idBC'], $Nhanvien['tenBC'], $Nhanvien['diaChi']);
            header("Location: QuanLyDon_NhanVien.php?idNhanVien=" . urlencode($Nhanvien['idNV']));
            exit();
        }
    } else if ($action == 'nhanhang') {

        VanDon::XacNhanVD_BuuCuc($pdo, $idVD, 'Đã nhận hàng', $NhanVienDangNhap['idNV'], $NhanVienDangNhap['idBC'], $NhanVienDangNhap['tenBC'], $NhanVienDangNhap['diaChi']);
        ...
        if (isset($_GET['idBC'])) {
            ...
            VanDon::VanChuyen($pdo, $idVD, 'Vận chuyển',  $NhanVienDangNhap['hoTen'], $BuuCuc['idBC'], $BuuCuc['tenBC'], $BuuCuc['diaChi']);
        }
    }
}
                            ...
                            <div class="tab-pane fade  <?= ($active == null || $active == 'ChoXacNhan')  ? 'show active' : '';
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td><a <?= !isset($_GET['idNhanVien']) ? 'class="disabled"' : '' ?> href="QuanLyDon_NhanVien.php?action=xacnhan&idNhanVien=<?= $Nhanvien['idNV'] ?>&idVD=<?= $order['idVD'] ?>">
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-end">
                                        ...
                                        <?php for ($i = 1; $i <= $total_pages_ChoXacNhan; $i++) : ?>
                                            <li class="page-item  <?= $i == $page ? 'active' : '' ?>">
                                                <a class="page-link" href="QuanLyDon_NhanVien.php?page=<?= $i ?>&active=ChoXacNhan"><?= $i ?></a>
                                            </li>
                                        <?php endfor; ?>
                                        <li class="page-item <?= $page >= $total_pages_ChoXacNhan ? 'disabled' : '' ?>">
                                            <a class="page-link" href="QuanLyDon_NhanVien.php?page=<?= $page + 1 ?>&active=ChoXacNhan">Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            </div>
                            <div class="tab-pane fade  <?= $active === 'DangLay'  ? 'show active' : '';  $page = $active !== 'DangLay' ? 1 : ($_GET['page']); ?>" id="pills-contact" role="tabpanel" aria-labelledby="pills-contact-tab">
                                <table class="table">
                                    <thead>
                                        <tr>
                                            <th scope="col">STT</th>
                                    ...
                                    <tbody>
                                        ...
                                        $DangLay = VanDon::getDonHangTrangThai($pdo, $NhanVienDangNhap['idBC'], $limit, $offset, 'Đang lấy');
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-end">
                                        <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                                            <a class="page-link" href="QuanLyDon_NhanVien.php?page=<?= $page - 1 ?>&active=DangLay">Previous</a>
                                        </li>
                                        <?php for ($i = 1; $i <= $total_pages_DangLay; $i++) : ?>
                                            <li class="page-item  <?= $i == $page ? 'active' : '' ?>">
                                                <a class="page-link" href="QuanLyDon_NhanVien.php?page=<?= $i ?>&active=DangLay"><?= $i ?></a>
                                            </li>
                                        <?php endfor; ?>
                                        <li class="page-item <?= $page >= $total_pages_DangLay ? 'disabled' : '' ?>">
                                            <a class="page-link" href="QuanLyDon_NhanVien.php?page=<?= $page + 1 ?>&active=DangLay">Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            </div>
                            <div class="tab-pane fade  <?= $active === 'LayThanhCong'  ? 'show active' : '';
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        ...
                                        $STT = 1;
                                        $offset = ($page - 1) * $limit;;
                                        $LayThanhCong = VanDon::getDonHangTrangThai($pdo, $NhanVienDangNhap['idBC'], $limit, $offset, 'Chuyển đến bưu cục');
